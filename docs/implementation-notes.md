...
same target API and covered by registry invariant tests. The compiler resolves a
`NativeTargetContext` once per compile so backend selection and platform
constants come from the same target metadata. Linux syscall numbers and OS ABI
constants stay behind a
