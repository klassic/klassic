...
| Native build command (`klassic build [--target <target>] <file> -o <output>`) | required | `tests/sample_programs.rs`, `tests/cross-exec/`, `docs/native-coverage.md` | partial | `cargo test` builds and runs every `test-programs/*.kl` and `examples/*.kl` on the host and cross-builds them for the other tier-0 targets; `tests/cross-exec/` compares each fixture's output against the evaluator's on real arm64 and Windows in CI. The per-construct coverage this row used to enumerate lives in `docs/native-coverage.md`. |
