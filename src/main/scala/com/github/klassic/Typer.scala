...
class Typer {
  ...
  def typeCheck(node: AST, environment : TypeEnvironment): TypeDescription = {
    node match {
      ...
      case AST.IntNode(_, _) => IntType
      case AST.ShortNode(_, _) => ShortType
      case AST.ByteNode(_, _) => ByteType
      case AST.LongNode(_, _) => LongType
      case AST.FloatNode(_, _) => FloatType
      case AST.DoubleNode(_, _) => DoubleType
      case AST.BooleanNode(_, _) => BooleanType
      ...
      case AST.FunctionDefinition(location, name, func, cleanup) =>
        ...
        UnitType
      case AST.FunctionCall(location, func, params) =>
