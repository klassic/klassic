...
object Parser {
  ...
  val bool: P[Ast.BoolValue] = P(("true".!.map{_ => true} | "false".!.map{_ => false}).map{Ast.BoolValue(_)})
  val identifier: P[Ast.Identifier] = P{
    (identifierFirst ~ (identifierFirst | P(CharIn('0'to'9'))).rep(0)).!.map{Ast.Identifier(_)}
  }
