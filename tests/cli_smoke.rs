...
#[cfg(all(target_os = "linux", target_arch = "x86_64"))]
#[test]
fn builds_native_executable_for_gc_length_reader_overflow_runtime_errors() {
    let cases = [
        (
            r#"val xs = __gc_list_int(0)
__gc_write(xs, 0, 1152921504606846460)
__gc_list_int_len(xs)
"#,
            "__gc_list_int_len",
...
"#,
    ...
    for (index, (source, builtin_name, diagnostic)) in cases.iter().enumerate() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("time should be monotonic")
            .as_nanos();
        let source_path = std::env::temp_dir().join(format!(
            "klassic-native-gc-length-reader-overflow-{index}-{unique}.kl"
        ));
        let output_path = std::env::temp_dir().join(format!(
            "klassic-native-gc-length-reader-overflow-{index}-{unique}"
        ));
        fs::write(&source_path, source).expect("source should write");
        ...
        assert!(
            build.status.success(),
            "gc length reader overflow build failed for {builtin_name}\nstdout:\n{}\nstderr:\n{}",
            String::from_utf8_lossy(&build.stdout),
            String::from_utf8_lossy(&build.stderr)
        );
...
#[cfg(all(target_os = "linux", target_arch = "x86_64"))]
#[test]
fn builds_native_executable_for_gc_string_replace_size_overflow_runtime_errors() {
    let source = r#"val to = __gc_string_alloc(0)
__gc_write(to, 0, 9223372036854771666)
__gc_string_replace(__gc_string("aa"), __gc_string("a"), to)
"#;
    let unique = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("time should be monotonic")
        .as_nanos();
    let source_path = std::env::temp_dir().join(format!(
        "klassic-native-gc-string-replace-overflow-{unique}.kl"
    ));
    let output_path = std::env::temp_dir().join(format!(
        "klassic-native-gc-string-replace-overflow-{unique}"
    ));
    fs::write(&source_path, source).expect("source should write");
    ...
    assert!(
        build.status.success(),
        "gc string replace overflow build failed\nstdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&build.stdout),
        String::from_utf8_lossy(&build.stderr)
    );
    ...
    assert_eq!(
        run.status.code(),
        Some(1),
        "__gc_string_replace should fail"
    );
    assert!(run.stdout.is_empty());
    assert_eq!(
        String::from_utf8_lossy(&run.stderr),
        format!(
            "{}:3:1: __gc_string_replace allocation size overflow\n",
            source_path.display()
        )
    );
}
